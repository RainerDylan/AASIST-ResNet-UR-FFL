...
def main():
    ...
    print(f"Initiating Standard Baseline AASIST Training on {device}")
    ...
    # Standard AASIST Protocol: 100 Epochs with Cosine Annealing
    total_epochs = 100
    ...
    for epoch in range(total_epochs):
        ...
        # Save baseline weights if EER improves, otherwise trigger early stopping tracker
        if val_eer < best_eer:
            best_eer = val_eer
            epochs_no_improve = 0
            torch.save(model.state_dict(), "aasist_baseline_best.pth")
            print("  -> Baseline EER Improved. New best model saved.")
        else:
            epochs_no_improve += 1
    ...
    plt.tight_layout()
    plt.savefig("aasist_baseline_metrics.png", dpi=300)
    print("Graph saved as 'aasist_baseline_metrics.png'.")
