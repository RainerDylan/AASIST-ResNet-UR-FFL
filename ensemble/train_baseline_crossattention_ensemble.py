"""
train_baseline_crossattention_ensemble.py
Cold-Start End-to-End Cross-Attention Ensemble — Revised v3

═══════════════════════════════════════════════════════════════════════════════
COMPLETE DIAGNOSIS OF ALL OBSERVED FAILURES (from plot + 2021 DF eval)
═══════════════════════════════════════════════════════════════════════════════

FAILURE 1 — 37% EER on ASVspoof 2021 DF / Val loss oscillation / Train EER→0% fast
  ─────────────────────────────────────────────────────────────────────────────
  Root cause A — Insufficient base-model supervision (AUX_W=0.40 total):
    0.20 effective weight per base model is too small during cold-start.
    ResNet-SimAM converges faster than AASIST (simpler 2D CNN vs. complex
    graph attention). With near-equal loss weighting from epoch 1, the fusion
    head receives improving ResNet embeddings but stagnant AASIST embeddings,
    and learns to discount AASIST. The result: ResNet alone drives decisions.
    ResNet then memorises ASVspoof 2019 LA spectral/codec fingerprints of the
    specific spoofing algorithms (A01–A19). These fingerprints do not appear
    in 2021 DF attacks → catastrophic generalisation failure (37% EER,
    missing 45% of spoofs at threshold 0.7466).

    FIX A — Phased loss weights:
      Phase 1 (epochs 1–PHASE1_END=20): META_W=0.25, AUX_W=0.75
        · Both base models receive 0.375 effective weight each — strong enough
          for simultaneous cold-start learning.
        · Fusion head gets gradient but doesn't dominate until bases converge.
      Phase 2 (epochs 21+): META_W=0.65, AUX_W=0.35
        · Fusion head takes over once AASIST and ResNet have built meaningful
          latent representations that are worth combining.
      The phase transition at epoch 20 matches typical AASIST convergence time
      for the ASVspoof 2019 LA training set (observed in prior single-model runs).

  Root cause B — No spectral regularisation on ResNet's mel-spectrogram input:
    Without augmentation, ResNet is free to memorise specific frequency bands
    and time-domain patterns that are artifact-correlated in 2019 LA codec
    chains (e.g., the synthesis residuals of A01 neural TTS, A06 WaveNet).
    These are entirely absent from 2021 DF attacks, which use different
    synthesis pipelines. ResNet's high AUC on 2019 LA dev (≈1.0) is therefore
    misleading — it reflects dataset-specific memorisation, not generalisation.

    FIX B — SpecAugment (Park et al. Interspeech 2019):
      T.FrequencyMasking(freq_mask_param=15, iid_masks=True): independently
        zeros up to 15 (out of 80) mel frequency bins per sample per forward
        pass, forcing ResNet to rely on the remaining spectral context.
      T.TimeMasking(time_mask_param=25, iid_masks=True): zeros up to 25
        consecutive time frames, preventing memorisation of temporal artifacts.
      Applied after amp_to_db during training only. Validation uses clean
        spectrograms to measure true detection performance.

FAILURE 2 — Ongoing val loss oscillation even after epoch 30
  ─────────────────────────────────────────────────────────────────────────────
  Root cause: insufficient regularisation.
    label_smoothing=0.05 allows the model to assign high confidence to
    training patterns. At near-zero train loss, any gradient step from the
    optimizer risks overshooting and perturbing validation operating point.

    FIX C — Increased regularisation:
      label_smoothing: 0.05 → 0.10 (FocalLoss)
      weight_decay:    1e-4 → 3e-4 (AdamW)
      dropout (fuser): 0.30 → 0.35 (CrossAttentionFuser)
    Together these flatten the loss landscape near the optimum, reducing the
    amplitude of per-epoch oscillation in val loss and EER.

═══════════════════════════════════════════════════════════════════════════════
UNCHANGED FROM v2
═══════════════════════════════════════════════════════════════════════════════
· Cold-start end-to-end: AASIST, ResNet, and CrossAttentionFuser all randomly
  reinitialised. No pretrained weights loaded.
· AASIST in FP32 (SincNet sinc-function NaN safety).
· ResNet + CrossAttentionFuser in FP16 (torch.amp.autocast).
· FP32 cast before FocalLoss (mixed-precision numerical safety).
· Pre-LN TransformerEncoderLayer (norm_first=True) for stable cold-start flow.
· 5-epoch linear warmup (0.1×LR → LR) → cosine annealing → 1e-7.
· PATIENCE=25, TOTAL_EPOCHS=100.
· Hook-based pre-classifier embedding extraction (inp[0] of aasist.fc / resnet.fc).
· NaN/Inf batch guard: skips corrupted batches without poisoning weights.
...
Devlin J. et al. (2019) NAACL: BERT CLS-token pooling.
"""
